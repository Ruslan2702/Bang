...
std::string int2ipv4(uint32_t ip) {
    char buf[128];
    snprintf(buf, sizeof(buf), "%u.%u.%u.%u", ip & 0xFF, (ip & 0xFF00) >> 8, (ip & 0xFF0000) >> 16,
             (ip & 0xFF000000) >> 24);
    return buf;
}
...
namespace {
    ...
    void set_non_blocked_impl(int sd, bool opt) noexcept(false) {
...
void Socket::setNonBlocked(bool opt) noexcept(false) {
...
void Socket::setReuseAddr(int sd) noexcept(false) {
    int yes = 1;
...
bool Socket::hasData() noexcept(false) {
    char buf[1];
